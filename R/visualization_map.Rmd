...
```{r worldmap}
pl <- ggplot() + 
  geom_polygon(data = world_data2, aes(x = long, y = lat, group = group, fill = log(Count))) +
...
```{r EUmap}
pl <- ggplot() + 
  geom_polygon(data = europe_data, aes(x = long, y = lat, group = group, fill = log(Count),text=region)) +
...
```{r usamap}
pl <- ggplot() + 
  geom_polygon(data = state_data2, aes(x = long, y = lat, group = group, fill = log(Count),text=region)) +
...
topicPerCountry <- data %>% group_by(affiliation,topic) %>%
  summarise(count=n_distinct(doi)) %>%
  group_by(affiliation) %>%
  summarise(topic=topic[count=max(count)])
